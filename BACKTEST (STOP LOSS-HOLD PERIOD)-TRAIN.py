...
asset_hourly_data = pd.read_csv(f'BTC_USDT_1h_(2020-2024)-TRAIN.csv')
...
for day in signals.index:
    ...
    if pd.to_datetime(time) > dt.datetime(2020, 2 ,1) and pd.to_datetime(time) >= pd.to_datetime(exit_time):
